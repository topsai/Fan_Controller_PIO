...
| 接收端绑定 C3 基础发射端 MAC | `AC:EB:E6:44:D5:54` |
| 接收端绑定 S3 高级发射端 MAC | `48:CA:43:9A:A9:B0` |

接收端当前以 `docs/protocol-v2.md` 为准维护 active 控制源。C3 基础发射端和 S3 高级发射端可以同时开机；非 active 遥控器的普通控制包不会抢占，必须按钮1长按 3 秒发送接管请求后才会切换状态回包目标。
...
3. 类型是否正确。
4. 校验和是否正确。
5. 接收端还必须检查发送方 MAC 是否等于绑定的 C3 或 S3 发射端 MAC。
