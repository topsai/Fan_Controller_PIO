...
| GPIO7 | `BUTTON_1_PIN` | 输入上拉 | 按钮 1，本地设置页面入口 |
...
## 9. ESP32-S3R8 正式发射端占位引脚

当前 `s3_transmitter` 已开始作为正式高级版遥控器软件环境，但实体摇杆、按钮、档位开关和蜂鸣器硬件尚未定稿。以下 GPIO 仅为软件占位，后续画板后必须按最终原理图更新。

| GPIO | 代码名称 | 方向 | 说明 |
|---|---|---|---|
| GPIO1 | `JOYSTICK_PIN` | ADC 输入 | 占位：回中摇杆 |
| GPIO37 | `SWITCH_PIN_1` | 输入上拉 | 占位：档位 1，低电平有效 |
| GPIO38 | `SWITCH_PIN_2` | 输入上拉 | 占位：档位 2，低电平有效 |
| GPIO39 | `SWITCH_PIN_3` | 输入上拉 | 占位：档位 3，低电平有效 |
| GPIO35 | `BUTTON_1_PIN` | 输入上拉 | 占位：按钮 1，本地设置页面入口 |
| GPIO36 | `BUTTON_2_PIN` | 输入上拉 | 占位：按钮 2，远程蜂鸣 |
| GPIO42 | `BUZZER_PIN` | 输出 | 占位：本地蜂鸣器 |
