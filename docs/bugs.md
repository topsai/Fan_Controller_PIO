...
| BUG-012 | Watch | High | S3 正式端 LVGL 页面文字彩边且芯片严重发热 | 发热初步根因判断：正式端在接收端未接入时仍 100Hz ESP-NOW 空发、WiFi 15dBm、背光满亮、主循环 1ms 空转，和探针项目相比功耗显著更高；已改为断联 20Hz 搜索、已连接 100Hz 控制、WiFi 8.5dBm、CPU 160MHz、背光 140/255、LVGL handler 5ms、主循环 delay 5ms。文字彩边初步根因判断：8-bit 并口屏的 RGB565 字节序与 LVGL 默认输出不匹配；已设置 `LV_COLOR_16_SWAP=1` | 已编译、上传 COM7 并抓取新功耗配置启动日志；RGB565 字节交换固件已上传；仍需用户触摸温度复测和目视确认文字彩边是否改善 |
