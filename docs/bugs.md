...
| BUG-012 | Watch | High | S3 正式端 LVGL 页面文字撕裂且芯片严重发热 | 初步根因判断：正式端在接收端未接入时仍 100Hz ESP-NOW 空发、WiFi 15dBm、背光满亮、主循环 1ms 空转，和探针项目相比功耗显著更高；已改为断联 20Hz 搜索、已连接 100Hz 控制、WiFi 8.5dBm、CPU 160MHz、背光 140/255、LVGL handler 5ms、主循环 delay 5ms | 已编译、上传 COM7 并抓取新功耗配置启动日志；仍需用户触摸温度复测和目视确认文字撕裂是否改善 |
