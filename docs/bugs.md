...
| BUG-014 | Watch | High | S3 间歇性断联/接收端周期断联提示音 | 初步判断 S3 控制包发送依赖主 `loop()`，当 LVGL/LCD/I2C/串口造成主循环抖动时，接收端可能超过 2000ms 收不到控制包并进入 failsafe；同时 19.5dBm 可能增加发热和供电压降风险。已改为独立 FreeRTOS 控制发送任务，S3 WiFi TX 使用 15dBm，S3 状态回包超时保持 1000ms，接收端 failsafe 仍为 2000ms | native 单测覆盖链路配置；三端编译通过；COM7 已上传。COM7 10 秒日志连续 `[OK]`；COM10 10 秒日志持续 `THR:0 SPD:1 BTN:00` 和 PWM 76，未见 `[FAILSAFE]` |
